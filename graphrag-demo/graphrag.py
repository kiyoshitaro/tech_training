...
def graph_rag_pipeline(documents, query, chunk_size=600, overlap_size=100):
    ...
    elements = extract_elements_from_chunks(chunks) # 1m58 : 0.004$ - 1m27

    # Step 3: Summarize elements
    summaries = summarize_elements(elements) # ~0.12$ - 2m18 : 0.01$ - 1m31 : 0.004$ - 1m21
    ...
    community_summaries = summarize_communities(communities, graph) # ~0.5$ - 8m35 : 0.02$ - 8m28

    # Step 6: Generate answers from community summaries
    final_answer = generate_answers_from_communities(
        community_summaries, query) # ~0.33$  - 7m : 0.025$ - 10m19
